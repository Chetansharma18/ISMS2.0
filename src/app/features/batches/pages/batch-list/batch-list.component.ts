...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule, UiTableComponent],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <!-- Notification Toast Banner -->
    @if (toastMessage()) {
      <div class="fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-lg shadow-lg bg-[#131A4D] text-white text-xs font-semibold animate-in fade-in slide-in-from-top-4 duration-200">
        <svg class="w-4 h-4 text-green-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
        <span>{{ toastMessage() }}</span>
        <button (click)="toastMessage.set(null)" class="ml-3 text-slate-300 hover:text-white text-base font-bold leading-none">&times;</button>
      </div>
    }

    <div class="space-y-6 animate-in fade-in zoom-in-95 duration-200">
      
      <!-- Page Header (Government Standard) -->
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-2 border-b border-slate-200">
        <div>
          <h1 class="text-2xl font-extrabold text-[#131A4D] tracking-tight">Batch Management</h1>
          <p class="text-xs text-slate-500 mt-1">Rajasthan Skill & Livelihoods Development Corporation • Candidate Allocation Desk</p>
        </div>
        <a 
          *ngIf="authService.hasRole('TP_PIA')"
          routerLink="/batches/create"
          class="inline-flex items-center gap-2 bg-[#131A4D] text-white px-4 py-2 rounded text-xs font-bold hover:bg-[#0a0e29] transition shadow-xs shrink-0">
      ...
      <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-wrap gap-4 items-center justify-between">
        <div class="flex-grow max-w-md relative">
          ...
          <input 
            ...
            class="w-full pl-9 pr-4 py-2 border border-slate-300 rounded text-xs font-medium focus:ring-1 focus:ring-[#131A4D] focus:border-[#131A4D]" 
            placeholder="Search by Batch Code, Course Name, Center, or Scheme...">
        </div>
        <div class="text-xs font-semibold text-slate-500">
          Total Batches: <span class="font-bold text-[#131A4D]">{{ filteredBatches.length }}</span>
        </div>
      </div>

      <!-- Government UI Table Component -->
      <app-ui-table
        [columns]="columns"
        [data]="filteredBatches"
        emptyMessage="No training batches found matching criteria.">

        <ng-template #rowTemplate let-batch let-col="column">

          <!-- Batch Info -->
          <ng-container *ngIf="col.key === 'batchCode'">
            <div class="font-mono font-bold text-[#131A4D] text-xs">{{ batch.batchCode }}</div>
            <div class="text-xs text-slate-700 font-semibold mt-0.5">{{ batch.batchName }}</div>
          </ng-container>

          <!-- Course & Scheme -->
          <ng-container *ngIf="col.key === 'course'">
            <div class="font-bold text-slate-800 text-xs">{{ batch.courseName }}</div>
            <span class="inline-block mt-1 px-2 py-0.5 bg-slate-100 text-slate-700 text-[10px] font-bold rounded border border-slate-300 uppercase">
              {{ batch.scheme }}
            </span>
          </ng-container>

          <!-- SDC Center -->
          <ng-container *ngIf="col.key === 'sdc'">
            <div class="font-semibold text-slate-800 text-xs">{{ batch.sdcName }}</div>
            <div class="text-[10px] font-mono text-slate-500">{{ batch.sdcCode }}</div>
          </ng-container>
          ...
          <ng-container *ngIf="col.key === 'capacity'">
            <div class="w-44">
              <div class="flex justify-between items-center text-xs font-bold text-slate-700 mb-1">
                <span>{{ batch.mappedAspirantsCount }} / {{ batch.targetCapacity }} Aspirants</span>
                <span class="text-[10px] text-slate-500 font-semibold">{{ calculatePercent(batch) }}%</span>
              </div>
              <div class="w-full h-2 bg-slate-100 rounded overflow-hidden border border-slate-200">
                <div 
                  class="h-full transition-all duration-300"
                  [ngClass]="batch.mappedAspirantsCount >= batch.targetCapacity ? 'bg-amber-500' : 'bg-emerald-600'"
          ...
          <ng-container *ngIf="col.key === 'duration'">
            <div class="text-xs text-slate-600 font-medium leading-tight">
              <div><span class="text-slate-400">From:</span> {{ batch.startDate }}</div>
              <div><span class="text-slate-400">To:</span> {{ batch.endDate }}</div>
            </div>
          </ng-container>
          ...
          <ng-container *ngIf="col.key === 'status'">
            <span 
              ...
              [ngClass]="{
                'bg-green-50 text-green-700 border-green-300': batch.status === 'ONGOING',
                'bg-blue-50 text-blue-700 border-blue-300': batch.status === 'APPROVED',
                'bg-slate-100 text-slate-700 border-slate-300': batch.status === 'DRAFT',
                'bg-purple-50 text-purple-700 border-purple-300': batch.status === 'COMPLETED'
              }">
              {{ batch.status }}
            </span>
          </ng-container>
          ...
          <ng-container *ngIf="col.key === 'action'">
            <div class="flex justify-end">
              <button 
                ...
                class="px-3 py-1.5 bg-[#131A4D] hover:bg-[#0a0e29] text-white text-xs font-bold rounded shadow-xs transition-all flex items-center gap-1.5 cursor-pointer">
                <svg class="w-3.5 h-3.5 text-blue-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
      ...
      <!-- ════════════════════════════════════════════════════════════════ -->
      <!-- MODAL: ASPIRANT MAPPING & UNMAPPING DESK                         -->
      <!-- ════════════════════════════════════════════════════════════════ -->
      @if (activeMappingBatch()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
          <div class="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-3xl overflow-hidden animate-in zoom-in-95 duration-150">
            
            <!-- Modal Header (Official Govt Navy) -->
            <div class="bg-[#131A4D] px-6 py-4 text-white flex justify-between items-center border-b border-slate-700">
              <div>
                <h3 class="text-base font-bold flex items-center gap-2">
                  <span>Batch Candidate Allocation Desk</span>
                  <span class="bg-white/10 px-2.5 py-0.5 rounded text-xs font-mono font-bold">{{ activeMappingBatch()?.batchCode }}</span>
                </h3>
            ...
            <div class="bg-slate-100 border-b border-slate-200 px-6 pt-3 flex gap-6">
              <button 
                ...
                <span>➕ Eligible Candidates to Map</span>
                <span class="px-2 py-0.5 text-[10px] rounded-full bg-emerald-100 text-emerald-800 font-bold border border-emerald-300">
                  {{ eligibleAvailableCount }} Available
                </span>
              </button>
              ...
              <button 
                ...
                class="pb-3 text-xs font-bold border-b-2 transition-all flex items-center gap-2"
                [ngClass]="activeModalTab() === 'UNMAP' ? 'border-purple-700 text-purple-800' : 'border-transparent text-slate-500 hover:text-slate-700'">
                <span>👥 Currently Mapped Candidates</span>
                <span class="px-2 py-0.5 text-[10px] rounded-full bg-purple-100 text-purple-800 font-bold border border-purple-300">
                  {{ currentlyMappedCount }} Mapped
                </span>
              </button>
            </div>
            ...
            <div class="px-6 py-3 bg-white border-b border-slate-200">
              <input 
                ...
                class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded text-xs font-medium focus:ring-1 focus:ring-[#131A4D] focus:border-[#131A4D]"
                [placeholder]="activeModalTab() === 'MAP' ? 'Search eligible candidates by name, reg no, or aadhaar...' : 'Search currently mapped candidates...'">
            </div>

            <!-- TAB 1: ELIGIBLE CANDIDATES TO MAP -->
            <div *ngIf="activeModalTab() === 'MAP'" class="p-6 max-h-[50vh] overflow-y-auto">
              <table class="w-full text-xs text-left border-collapse">
                <thead class="bg-slate-100 text-slate-700 font-bold uppercase border-b border-slate-200">
                  <tr>
                    <th class="w-10 px-3 py-2.5 text-center">Select</th>
                    <th class="px-3 py-2.5">Reg No</th>
                    <th class="px-3 py-2.5">Aspirant Name</th>
                    <th class="px-3 py-2.5">Aadhaar</th>
                    <th class="px-3 py-2.5">Mobile</th>
                    <th class="px-3 py-2.5">Qualification</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-slate-200">
                  <tr *ngIf="eligibleAspirants.length === 0">
                    <td colspan="6" class="px-4 py-8 text-center text-slate-400">
                      No eligible approved candidates available to map.
                    </td>
                  </tr>

                  <tr *ngFor="let t of eligibleAspirants" class="hover:bg-slate-50" [ngClass]="{'bg-emerald-50/60': selectedToAssign.has(t.id)}">
                    <td class="px-3 py-2.5 text-center">
                      <input 
                        type="checkbox" 
                        [checked]="selectedToAssign.has(t.id)"
                        (change)="toggleAssign(t.id)"
                        class="rounded border-slate-300 text-[#131A4D] focus:ring-[#131A4D] cursor-pointer w-4 h-4">
                    </td>
            ...
            <!-- TAB 2: CURRENTLY MAPPED CANDIDATES (UNMAP DESK) -->
            <div *ngIf="activeModalTab() === 'UNMAP'" class="p-6 max-h-[50vh] overflow-y-auto">
              <table class="w-full text-xs text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-200">
                  <tr *ngIf="mappedAspirants.length === 0">
                    <td colspan="5" class="px-4 py-8 text-center text-slate-400">
                      No candidates are currently mapped to this batch.
                    </td>
                  </tr>

                  <tr *ngFor="let t of mappedAspirants" class="hover:bg-purple-50/40">
                    <td class="px-3 py-2.5 font-mono font-bold text-[#131A4D]">{{ t.registrationNo }}</td>
                    <td class="px-3 py-2.5 font-bold text-slate-800">{{ t.name }}</td>
                    <td class="px-3 py-2.5 font-mono text-slate-600">{{ t.aadhaarNo }}</td>
                    <td class="px-3 py-2.5 font-semibold text-slate-700">{{ t.mobile }}</td>
                    ...
                    <td class="px-3 py-2.5 text-right">
                      <button 
                        (click)="unmapSingle(t)"
                        class="px-3 py-1 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 font-bold rounded text-[11px] transition-colors inline-flex items-center gap-1">
            ...
            <!-- Modal Footer -->
            <div class="bg-slate-50 px-6 py-3.5 border-t border-slate-200 flex justify-between items-center">
              <button (click)="activeMappingBatch.set(null)" class="px-4 py-1.5 bg-white border border-slate-300 text-slate-700 text-xs font-bold rounded hover:bg-slate-100 transition-colors">
                Close
              </button>
              ...
              <div *ngIf="activeModalTab() === 'MAP'">
                <button 
                  ...
                  Confirm & Map Candidates ({{ selectedToAssign.size }})
                </button>
              </div>
            </div>
...
export class BatchListComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    { key: 'batchCode', label: 'Batch Info' },
    { key: 'course', label: 'Course & Scheme' },
    { key: 'sdc', label: 'Center (SDC)' },
    { key: 'capacity', label: 'Capacity & Mapped' },
    { key: 'duration', label: 'Batch Duration' },
  ...
  mapSelectedAspirants() {
    ...
    this.traineeService.bulkAssignToBatch(ids, batch.batchCode).subscribe({
      next: () => {
        ...
        this.showToast(`✓ ${ids.length} candidate(s) successfully mapped to batch ${batch.batchCode}`);
        this.selectedToAssign.clear();
  ...
  unmapSingle(trainee: Trainee) {
    ...
    this.traineeService.unmapFromBatch(trainee.id).subscribe({
      next: () => {
        const newCount = this.currentlyMappedCount;
        this.batchService.updateMappedCount(batch.batchCode, newCount);
        this.showToast(`✓ ${trainee.name} (${trainee.registrationNo}) unmapped from batch ${batch.batchCode}`);
      }
    });
  }
